...
class VQCEnv(gym.Env):
    ...
    def step(self, action):  # needs changing (need to deal with parameters)random
        ...
        if gate==1:
            ...
            print("error in step(): invalid action. ", qubit, gate, action)
            exit()
        ...
        stats_buffer['gate'].append(self.decode_gate[gate])
        stats_buffer['qubit'].append(qubit)
...
def select_action(state):
    ...
    st = tuple(state.squeeze(0).to(dtype = torch.int32).tolist())
    st = state_to_idx[st]

    if sum(stats_buffer["Ti(n)"][st])<10:
        action = sum(stats_buffer["Ti(n)"][st])
        return action
    
    action = np.argmax(q_table[st] + 2*(np.log(num_episodes+1)/stats_buffer["Ti(n)"][st])**0.5)
    return action


def q_update(st,a,next_st,rew):  #perform q_ucb
    ...
    if sum(stats_buffer["Ti(n)"][st])<10:
        ...
        q_table[st][a] = ((q_table[st][a] * stats_buffer["Ti(n)"][st][a]) + rew)/(stats_buffer["Ti(n)"][st][a]+1)


    stats_buffer["Ti(n)"][st][a]+=1
    stats_buffer["QTable"][i_episode].append(q_table.tolist())
