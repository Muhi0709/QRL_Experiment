...
wandb_run = wandb.init(
    ...
    name = "spsa_no_pretrain_from_102_lr=0.001",
    project="QRL_spsa",
    entity="ep19b005",
    resume=True,
)
            ...
            for _,v in prev_action.items():
                if v!= float("inf"):
                    if v not in action_ch:
                        ...
                        action_ch.remove(v)
            return torch.tensor([[np.random.choice(action_ch)]], dtype = torch.long)
        
        else:
            return torch.tensor([[np.random.choice([n_actions-1,n_actions-2])]], dtype = torch.long)
