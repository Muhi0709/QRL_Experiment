...
handler = logging.FileHandler("./logs_prevbest_no_pretrain/exp_1.log", "a")
...
class VQCEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        state_np = np.vstack((np.real(state_np),np.imag(state_np)))
    ...
    def construct_tensor_state(self,temp):

        cur_depth = 0
        exceeded_flag=0
        gate_count = 0

        temp_state = np.zeros((self.max_depth,self.num_qubits+3,self.num_qubits))
        ...
        for inst in temp.data:
            ...
            if inst[0].name == "cx":
                ...
                if sum(temp_state[cur_depth,:,target])==0:
                    temp_state[cur_depth][control][target]=1
                else:
                    cur_depth+=1
                ...
                qubit = inst[1][0].index
                if sum(temp_state[cur_depth,:,qubit])==0:
                    temp_state[cur_depth][self.num_qubits][qubit]=1
                else:
                    cur_depth+=1
                    if cur_depth==self.max_depth:
                        exceeded_flag+=1
                ...
                qubit = inst[1][0].index
                if sum(temp_state[cur_depth,:,qubit])==0:
                    temp_state[cur_depth][self.num_qubits+1][qubit]=1
                else:
                    cur_depth+=1
                    if cur_depth==self.max_depth:
                        exceeded_flag+=1
                ...
                if sum(temp_state[cur_depth,:,qubit])==0:
                    temp_state[cur_depth][self.num_qubits+2][qubit]=1
                else:
                    cur_depth+=1
                    if cur_depth==self.max_depth:
                        exceeded_flag+=1
    ...
    def step(self, action):  # needs changing (need to deal with parameters)random
      ...
      # remove the last column completely...
        ...
        a = self.construct_tensor_state(temp)
        if a:
            self.state, self.depth = a
            self.ansatz = deepcopy(temp)
...
class DQN(nn.Module):
    def __init__(self, n_observations, n_actions):
        ...
        self.layer7 = nn.Linear(1024,n_actions)
...
def make_checkpoint(obj):
    # 162000: 45 hours
    with open("./checkpoints_prevbest/checkpoint_1.pkl", "wb") as f:
        pickle.dump(obj, f)
...
if os.path.exists("./checkpoints_prevbest/checkpoint_1.pkl"):
    with open("./checkpoints_prevbest/checkpoint_1.pkl", "rb") as f:
...
for i_episode in range(start, num_episodes):
    ...
    filepath = "./json_objs_prevbest_no_pretrain/exp_1.json"
    ...
    if time() - code_start > 144000:  # 162000
        checkpoint = {
            "start": i_episode + 1,
            "env": env,
