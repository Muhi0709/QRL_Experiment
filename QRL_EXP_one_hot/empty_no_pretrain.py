...
handler = logging.FileHandler("./logs_empty_no_pretrain/exp_1.log", "a")
...
class VQCEnv(gym.Env):
    ...
    def __init__(self, vqc_experiment_data, rl_agent_data, render_mode="human"):
        ...
        self.state = np.zeros((self.max_depth,self.num_qubits+3,self.num_qubits)
        ...
        self.action_space = Discrete(
            6 * self.num_qubits + 2 * self.num_qubits * (self.num_qubits - 1) + 2
        )

        self.observation_space = Box(
            low=-5, high=5, shape=(self.max_depth*(self.num_qubits)*(self.num_qubits+3) + 1,), dtype=np.intc
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.reset_state == "prev_best":
            ...
            if sample>0.5 and prev_best_env["depth"] < self.max_depth:
                ...
                self.num_gates = prev_best_env['num_gates']
        ...
        flattened_state = torch.cat(
            (torch.reshape(state_tf,(-1,)), torch.tensor([self.best_f1_score]))
        ).to(device=device)
    ...
    def decode_action(self, action):
        ...
        if action < 6 * self.num_qubits:
            ...
            if self.prev_action[self.num_qubits*pos + qubit] == float("inf"):
                ...
                self.prev_action[self.num_qubits + qubit] =  action if pos else  action + (3*self.num_qubits)
                self.prev_action[qubit] = action if not pos else action-(3*self.num_qubits)
            ...
            if self.prev_action[self.num_qubits*pos + qubit[1]] == float("inf"):
                a = action - 6*self.num_qubits
                self.prev_action[self.num_qubits + qubit[1]] =  action if pos else  action + ((self.num_qubits-1)*self.num_qubits)
                self.prev_action[qubit[1]] = action if not pos else action-(self.num_qubits*(self.num_qubits-1))
    ...
    def step(self, action):  # needs changing (need to deal with parameters)random
      ...
      # remove the last column completely...
                ...
                if sum(self.state[0,:,qubit])==0:

                    self.num_gates+=1
                    self.state[0][self.num_qubits+gate-1][qubit]=1
                    ...
                    if self.depth<self.max_depth:
                        self.num_gates+=1
                        temp = self.state[:self.depth]
                        self.state[0] = np.zeros((self.num_qubits+3,self.num_qubits))
                        self.state[0,self.num_qubits+gate-1,qubit]=1
                        self.state[1:] = temp
                ...
                if sum(self.state[self.depth,:,qubit])==0:
                    self.num_gates+=1
                    self.state[self.depth][self.num_qubits+gate-1][qubit]=1
            ...
            if pos == 0:
                ...
                if sum(self.state[0,:,qubit[1]])==0:
                    self.num_gates+=1
                    self.state[0][qubit[0]][qubit[1]]=1
                ...
                if sum(self.state[self.depth,:,qubit[1]])==0:

                    self.num_gates+=1

                    self.state[self.depth][qubit[0]][qubit[1]]=1
            ...
            if removed[0].name == "rx":
                ...
                gate = 4
                qubit = (removed[1][0].index,removed[1][1].index)

            if pos==1:
                if gate<=3:
                    self.state[self.depth,self.num_qubits+gate-1,qubit]=0
                else:
                    self.state[self.depth,qubit[0],qubit[1]]=0
                
                if (self.state[self.depth] == np.zeros((self.num_qubits+3,self.num_qubits))).all():
                    self.depth-=1
            
            else:
                if gate<=3:
                    self.state[0,self.num_qubits+gate-1,qubit]=0
                else:
                    self.state[0,qubit[0],qubit[1]]=0
                
                if (self.state[0] == np.zeros((self.num_qubits+3,self.num_qubits))).all():
                    self.state[:self.depth] = self.state[1:self.depth+1] 
                    self.state[self.depth] = np.zeros((self.num_qubits+3,self.num_qubits))
                    self.depth-=1
        ...
        stats_buffer['gate'][i_episode].append(self.decode_gate[gate])
        stats_buffer['qubit'][i_episode].append(qubit)
    ...
    def delta_reward(self, f1_score, action_type):
        ...
        if action_type == "single-qubit" or action_type == "two-qubit":
            reward = (-0.01 + (f1_score - self.best_f1_score))
        elif action_type == "removal":
            reward = (0.01 + (f1_score - self.best_f1_score)) if self.num_gates != 0 else 0
...
def make_checkpoint(obj):
    # 162000: 45 hours
    with open("./checkpoints_empty/checkpoint_1.pkl", "wb") as f:
        pickle.dump(obj, f)
...
if os.path.exists("./checkpoints_empty/checkpoint_1.pkl"):
    with open("./checkpoints_empty/checkpoint_1.pkl", "rb") as f:
...
if checkpoint:
    ...
    objective_values = []
    prev_best_env = {
        "state": np.zeros((env.max_depth,env.num_qubits+3,env.num_qubits)),
        "depth": 0,
...
def select_action(state):
    ...
    if sample>=eps_threshold:
        ...
        stats_buffer['greed_rand'][i_episode].append("Greedy")
        for _,v in env.prev_action.items():
            if v != float("inf"):
                action_mask[0][v] = -float("inf")
        
        if env.num_gates <= 1 or (env.num_params==1 and (env.ansatz.data[0][0].name!='cx' or env.ansatz.data[-1][0].name!='cx')):
    ...
    # Using the epsilon greedy approach with a high epsilon value in the beginning to encourage exploration
        ...
        random_flag = True
        stats_buffer["greed_rand"][i_episode].append("Random")
...
if rl_agent_data['use_pre_trained']:
    ...
    minimum = 1/(policy_net.layer6.in_features**0.5)
    layer_6_w = ((torch.rand((n_actions,policy_net.layer6.in_features)) * (2* minimum))-minimum)
    layer_6_b = (torch.rand(n_actions)* (2*minimum) - minimum)

    del a['f1_layer.bias']
    del a['f1_layer.weight']

    a['layer6.weight'] = layer_6_w
    a['layer6.bias'] = layer_6_b
...
wandb_run = wandb.init(
    ...
    name = "empty_no_pretrain",
    project="QRL1_no_action_unitary_rep",
    entity="ep19b005",
    resume=True,
)
"""
...
if checkpoint:
    ...
    stats_buffer = {
        ...
        "n_actions": n_actions,
        "greed_rand": [[] for _ in range(num_episodes)],
        "gate": [[] for _ in range(num_episodes)],
        "qubit": [[] for _ in range(num_episodes)],
...
for i_episode in range(start, num_episodes):
    ...
    stats_buffer["ansatzes"][i_episode].append(state.squeeze(0).tolist())
    ...
    while True:
        ...
        if env.latest_f1_score > max_f1_score and env.reset_state=="prev_best":
            ...
            prev_best_env["gates"] = env.num_gates 
        ...
        if done:
            ...
            wandb.log({"Total_Reward": total_reward,
                       ...
                       "Removal steps": stats_buffer["Total_number_of_gates_removed"][i_episode]}
                       )
            """
    ...
    filepath = "./json_objs_empty_no_pretrain/exp_1.json"
