...
class VQCEnv(gym.Env):
    ...
    def __init__(self, vqc_experiment_data, rl_agent_data, render_mode="human"):
        ...
        self.observation_space = Box(
            low=-5, high=5, shape=(2*dim*dim + 1,), dtype=np.intc
        )
    ...
    def test_ansatz_and_get_f1(self):
        ...
        cur_ansatz = vqc.ansatz
        cur_ansatz = cur_ansatz.bind_parameters(self.param_values)
        job = execute(cur_ansatz, unitary_sim, shots=1)
        result = job.result()

        self.state = np.asarray(result.get_unitary(cur_ansatz))
